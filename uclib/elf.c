...
static int elf_section_sort_by_file_offset (elf_section_t * s1, elf_section_t * s2)
{
  i64 cmp = (i64) s1->header.file_offset - (i64) s2->header.file_offset;
  return cmp < 0 ? -1 : (cmp > 0 ? + 1 : 0);
}

static int elf_segment_sort_by_virtual_address (elf_segment_t * s1, elf_segment_t * s2)
{
  i64 cmp = (i64) s1->header.virtual_address - (i64) s2->header.virtual_address;
  return cmp < 0 ? -1 : (cmp > 0 ? + 1 : 0);
}
...
format_elf_main (u8 * s, va_list * args)
{
...

  {
    ...
    vec_sort (copy, (void *) elf_section_sort_by_file_offset);
...

  {
    ...
    vec_sort (copy, (void *) elf_segment_sort_by_virtual_address);
