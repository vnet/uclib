...
static char *
path_exists_and_is_executable (char * path)
{
  struct stat s;
  return (stat (path, &s) >= 0
	  && S_ISREG (s.st_mode)
	  && 0 != (s.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH))) ? path : 0;
}
...
file_exists_and_is_executable (char * dir, char * file)
{
  ...
  yes = path_exists_and_is_executable (path) != 0;
  vec_free (path);
...
path_search (char * file)
{
  ...
  if (file[0] == '.' || file[0] == '/')
    return path_exists_and_is_executable (file) ? file : 0;
